...
let answer1 = numbers.reduce((x,y) => x - y, 200)
...
let redWine = data
.filter(x => x.wineColor === 'red')
.map(x => x.numBottles + 500)
.reduce((x,y) => x+y)
console.log('question 2:', redWine )
...
console.log('question 3:', nums1.reduce((x,y) => x + y) / nums1.length)
console.log(nums2.reduce((x,y) => x + y )/ nums2.length)
console.log(nums3.reduce((x,y) => x + y )/ nums3.length)
console.log('No idea how to do nums4')
//    console.log(nums4.reduce((x,y) => ? typeof y === 'string' x + y )/ nums3.length)
...
let businesses = [
      ...
      let newBusinesses = businesses.filter(x => x.startYear > 2000)
      console.log('question 4:', newBusinesses)
...
let ar1 = [1,2,3,4];
let ar2 = [1,2,undefined,6]
let ar3 = [1,3,25.5,4,32.9];
console.log('question 5:', ar1.map(x => x * 5))
console.log('I need help with this one:', ar2.map(x => (x === undefined) ? 'remove' : x * 5))
// 6.
